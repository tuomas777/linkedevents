...
NAMES_TO_IGNORE_BY_PROVIDER = {
    "Helsingin kaupunginteatteri": (
        "käsiohjelma",
    ),  # Certain events are not actual events.
    "SeaLife": ("kertaliput",),
...
)  # Certain events might already exist
...
class LippupisteImporter(Importer):
    ...
    def _cache_place_data(self):
        self.place_data_list = Place.objects.filter(
            data_source=self.tprek_data_source, deleted=False
        ).values("id", "name", "street_address_fi", "street_address_sv", "postal_code")
    ...
    def _cache_super_event_ids(self):
        qs = Event.objects.filter(
            data_source=self.data_source, super_event_type__isnull=False
        )
        self.super_event_ids_by_origin_id = {
            super_event.origin_id: super_event.id for super_event in qs
        }
    ...
    def _get_keywords_from_source_categories(self, source_categories):
        ...
        for category in source_categories:
            keyword_set = keyword_set.union(
                self._get_keywords_from_source_category(category)
            )
        return keyword_set
    ...
    def _get_place_id_from_source_event(self, source_event):
        ...
        for place_data in self.place_data_list:
            ...
            source_place_name_words = set(
                source_place_name.replace(",", " ")
                .replace("-", " ")
                .replace("&", " ")
                .split()
            )
            common_words = candidate_place_name_words & source_place_name_words
            different_words = candidate_place_name_words.symmetric_difference(
                source_place_name_words
            )
            if candidate_place_name and source_place_name and common_words != set():
                matches_by_partial_name[len(common_words)][len(different_words)].append(
                    place_id
                )
            ...
            if candidate_postal_code and source_postal_code == candidate_postal_code:
                ...
                is_partial_address_match = (
                    ...
                    and (
                        ...
                        or candidate_address_fi in source_address
                    )
                ) or (
                    candidate_address_sv is not None
                    and (
                        source_address in candidate_address_sv
                        or candidate_address_sv in source_address
                    )
                )
    ...
    def _import_event(self, source_event, events):
        ...
        superevent_source_id = get_namespaced_event_serie_id(
            source_event["EventSerieId"]
        )
                ...
                if any([getattr(subevent, "name_" + lang) for subevent in events]):
                    names[lang].extend(
                        [
                            ...
                            if getattr(subevent, "name_" + lang, None)
                        ]
                    )
            super_event_name = {}
            for lang in settings.LANGUAGES:
    ...
    def _synch_events(self, events):
        ...
        syncher_queryset = Event.objects.filter(
            end_time__gte=now, data_source=self.data_source, deleted=False
        )
